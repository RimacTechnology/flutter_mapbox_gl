...
typedef void OnFeatureTappedCallback(dynamic id, Point<double> point, LatLng coordinates);
...
class MapboxMapController extends ChangeNotifier {
  MapboxMapController({
    ...
    _mapboxGlPlatform.onScaleBeingPlatform.add((_) {
      _isCameraZooming = true;
      scaleStart.notifyListeners();
    });

    _mapboxGlPlatform.onScalePlatform.add((_) {
      scale.notifyListeners();
    });

    _mapboxGlPlatform.onScaleEndPlatform.add((_) {
      _isCameraZooming = false;
      scaleEnd.notifyListeners();
    });

    _mapboxGlPlatform.onCameraMoveStartedPlatform.add((_) {
      _isCameraMoving = true;
      cameraMoveStart.notifyListeners();
    });

    _mapboxGlPlatform.onCameraMovePlatform.add((cameraPosition) {
      _cameraPosition = cameraPosition;
      cameraMove.notifyListeners();
    });
  ...
  final ChangeNotifier scaleStart = ChangeNotifier();
  final ChangeNotifier scale = ChangeNotifier();
  final ChangeNotifier scaleEnd = ChangeNotifier();

  final ChangeNotifier cameraMoveStart = ChangeNotifier();
  final ChangeNotifier cameraMove = ChangeNotifier();
  final ChangeNotifier cameraMoveEnd = ChangeNotifier();
  ...
  /// True if the map camera is currently zooming.
  bool get isCameraZooming => _isCameraZooming;
  bool _isCameraZooming = false;
  ...
  Widget buildView(Map<String, dynamic> creationParams, OnPlatformViewCreatedCallback onPlatformViewCreated,
    ...
    return _mapboxGlPlatform.buildView(creationParams, onPlatformViewCreated, gestureRecognizers);
  }
  ...
  Future<void> addLayer(
    ...
    if (properties is FillLayerProperties) {
      addFillLayer(sourceId, layerId, properties, belowLayerId: belowLayerId, sourceLayer: sourceLayer, filter: filter);
    } else if (properties is LineLayerProperties) {
      addLineLayer(sourceId, layerId, properties, belowLayerId: belowLayerId, sourceLayer: sourceLayer, filter: filter);
      ...
      addRasterLayer(sourceId, layerId, properties, belowLayerId: belowLayerId, sourceLayer: sourceLayer);
    } else if (properties is HillshadeLayerProperties) {
      if (filter != null) {
        throw UnimplementedError("HillShadeLayer does not support filter");
      }
      addHillshadeLayer(sourceId, layerId, properties, belowLayerId: belowLayerId, sourceLayer: sourceLayer);
    } else {
